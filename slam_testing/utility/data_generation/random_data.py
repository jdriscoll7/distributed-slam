...
def create_random_dataset(translation_variance, rotation_variance, n_poses, file_name):
    ...
    truth_rotations = [rotation_matrix(np.random.uniform(-np.pi, np.pi)) for i in range(n_poses)]
    ...
    truth_states = [np.vstack([truth_coordinates[i], angle_from_rotation_matrix(truth_rotations[i])]) for i in range(n_poses)]

    # Turn everything into vertices and edges.
    vertices = [Vertex(i, truth_states[i]) for i in range(n_poses)]
